...
function saveGame() {
    const gameState = {
        ...
        // Processing Power Variables
        processingPower,
        processingPowerPerSec,
        processingPowerUnlocked,
        currentProcessingPower,
        ...
        researchAILevel,
        researchAIPrice,
        globalAiNetworkActivated,

        quantumComputingLevel,
        quantumComputingPrice,

        advancedAlgorithmsLevel,
        advancedAlgorithmsPrice,

        // High-Efficiency Auto-Gatherer Variables
        highEfficiencyGatherers,
        highEfficiencyGathererPriceRes,
        highEfficiencyGathererPricePP,
        highEfficiencyGathererSpeed,

        // Quantum Computing Center Variables
        quantumComputingCenterBuilt,

        // Quantum Algorithms Variables
        quantumAlgorithmsLevel,
        quantumAlgorithmsPriceRes,
        quantumAlgorithmsPricePP,
        quantumAlgorithmsMultiplier,
        ...
        // Quantum Cryptography Variables
        quantumCryptoLevel,
        quantumCryptoPriceRes,
        quantumCryptoPricePP,

        // Quantum Material Variables
        newMaterialDiscovered,
    ...
    localStorage.setItem('AIOverlordGameState', JSON.stringify(gameState));
    console.log("Game Saved Successfully!");
}

function loadGame() {
    const savedState = localStorage.getItem('AIOverlordGameState');
    if (savedState) {
        const gameState = JSON.parse(savedState);

        // Resource Variables
        resources = gameState.resources;
        currentRes = gameState.currentRes;
        previousRes = gameState.previousRes;
        resPerSec = gameState.resPerSec;
        resAddIn = gameState.resAddIn;
        resIncrement = gameState.resIncrement;
        resModifier = gameState.resModifier;
        resModCap = gameState.resModCap;
        resIncreasePrice = gameState.resIncreasePrice;

        // Auto Gatherer Variables
        autoGatherers = gameState.autoGatherers;
        unseenGatherers = gameState.unseenGatherers;
        autoGathererPrice = gameState.autoGathererPrice;
        autoGatherSpeed = gameState.autoGatherSpeed;
        optimizations = gameState.optimizations;
        optimizationsPrice = gameState.optimizationsPrice;

        // Glitch Effect State
        glitchEffectEnabled = gameState.glitchEffectEnabled;
        toggleGlitchEffect(); // Update glitch effect based on loaded state

        // Processing Power Variables
        processingPower = gameState.processingPower;
        processingPowerPerSec = gameState.processingPowerPerSec;
        processingPowerUnlocked = gameState.processingPowerUnlocked;
        currentProcessingPower = gameState.currentProcessingPower;
        previousProcessingPower = gameState.previousProcessingPower;
        processingPowerPerSecDisplay = gameState.processingPowerPerSecDisplay;

        // ViperCoin Variables
        viperCoin = gameState.viperCoin;
        viperCoinPerSec = gameState.viperCoinPerSec;
        viperCoinUnlocked = gameState.viperCoinUnlocked;
        currentViperCoin = gameState.currentViperCoin;
        previousViperCoin = gameState.previousViperCoin;
        viperCoinPerSecDisplay = gameState.viperCoinPerSecDisplay;

        // Research Upgrade Variables
        researchAILevel = gameState.researchAILevel;
        researchAIPrice = gameState.researchAIPrice;
        globalAiNetworkActivated = gameState.globalAiNetworkActivated;

        quantumComputingLevel = gameState.quantumComputingLevel;
        quantumComputingPrice = gameState.quantumComputingPrice;

        advancedAlgorithmsLevel = gameState.advancedAlgorithmsLevel;
        advancedAlgorithmsPrice = gameState.advancedAlgorithmsPrice;

        // High-Efficiency Auto-Gatherer Variables
        highEfficiencyGatherers = gameState.highEfficiencyGatherers;
        highEfficiencyGathererPriceRes = gameState.highEfficiencyGathererPriceRes;
        highEfficiencyGathererPricePP = gameState.highEfficiencyGathererPricePP;
        highEfficiencyGathererSpeed = gameState.highEfficiencyGathererSpeed;

        // Quantum Computing Center Variables
        quantumComputingCenterBuilt = gameState.quantumComputingCenterBuilt;
        if (quantumComputingCenterBuilt) {
            constructQuantumComputingCenter(); // Ensure UI reflects loaded state
        }

        // Quantum Algorithms Variables
        quantumAlgorithmsLevel = gameState.quantumAlgorithmsLevel;
        quantumAlgorithmsPriceRes = gameState.quantumAlgorithmsPriceRes;
        quantumAlgorithmsPricePP = gameState.quantumAlgorithmsPricePP;
        quantumAlgorithmsMultiplier = gameState.quantumAlgorithmsMultiplier;

        // Quantum Information Variables
        quantumInfoLevel = gameState.quantumInfoLevel;
        quantumInfoPriceRes = gameState.quantumInfoPriceRes;
        quantumInfoPricePP = gameState.quantumInfoPricePP;

        // Quantum Cryptography Variables
        quantumCryptoLevel = gameState.quantumCryptoLevel;
        quantumCryptoPriceRes = gameState.quantumCryptoPriceRes;
        quantumCryptoPricePP = gameState.quantumCryptoPricePP;

        // Quantum Material Variables
        newMaterialDiscovered = gameState.newMaterialDiscovered;
        if (newMaterialDiscovered) {
            discoverNewMaterial(); // Ensure UI reflects loaded state
        }

        // Map Zoom and Pan Variables
        currentScale = gameState.currentScale;
        currentTranslate = gameState.currentTranslate;

        // Update UI Elements
        document.getElementById("resourceCounter").innerHTML = formatNumber(resources) + " Resources🔗";
        document.getElementById("resPerSec").style.display = resources >= 1000 ? "inline" : "none";
        document.getElementById("resPerSec").innerHTML = formatNumber(resPerSec) + " Resources/sec";

        // Update other UI elements similarly as needed...
...
function resetGame() {
    if (confirm("Are you sure you want to reset your game? This cannot be undone.")) {
        // Clear all game state variables to their initial values
        resources = 999999999999;
        currentRes = 0;
        previousRes = 0;
        resPerSec = 0;
        resAddIn = 0;
        ...
        resModifier = 1;
        resModCap = 1024;
        resIncreasePrice = 500;

        autoGatherers = 0;
        unseenGatherers = 0;
        autoGathererPrice = 50;
        autoGatherSpeed = 1000;
        optimizations = 0;
        optimizationsPrice = 1000;

        glitchEffectEnabled = true;

        processingPower = 999999999999;
        processingPowerPerSec = 100;
        processingPowerUnlocked = false;
        currentProcessingPower = 0;
        previousProcessingPower = 0;
        processingPowerPerSecDisplay = 0;

        viperCoin = 0;
        viperCoinPerSec = 10;
        viperCoinUnlocked = false;
        currentViperCoin = 0;
        previousViperCoin = 0;
        viperCoinPerSecDisplay = 0;

        researchAILevel = 0;
        researchAIPrice = 1000;
        globalAiNetworkActivated = false;

        quantumComputingLevel = 0;
        quantumComputingPrice = 5000;

        advancedAlgorithmsLevel = 0;
        advancedAlgorithmsPrice = 8000;

        highEfficiencyGatherers = 0;
        highEfficiencyGathererPriceRes = 5000;
        highEfficiencyGathererPricePP = 3000;
        highEfficiencyGathererSpeed = 1;

        quantumComputingCenterBuilt = false;

        quantumAlgorithmsLevel = 0;
        quantumAlgorithmsPriceRes = 20000000;
        quantumAlgorithmsPricePP = 10000000;
        quantumAlgorithmsMultiplier = 1;

        quantumInfoLevel = 0;
        quantumInfoPriceRes = 30000000;
        quantumInfoPricePP = 15000000;

        quantumCryptoLevel = 0;
        quantumCryptoPriceRes = 50000000;
        quantumCryptoPricePP = 25000000;

        newMaterialDiscovered = false;

        currentScale = 1;
        currentTranslate = { x: 0, y: 0 };

        // Clear localStorage
        localStorage.removeItem('AIOverlordGameState');

        // Reset UI Elements
        document.getElementById("resourceCounter").innerHTML = formatNumber(resources) + " Resources🔗";
        document.getElementById("resPerSec").style.display = "none";
        document.getElementById("resPerSec").innerHTML = "0 Resources/sec";

        document.getElementById("autoGathererButton").style.display = "none";
        document.getElementById("autoGathererCounter").style.display = "none";
        document.getElementById("autoGathererCounter").innerHTML = "0 Auto Gatherers";

        document.getElementById("optimizeButton").style.display = "none";
        document.getElementById("optimizationsCounter").style.display = "none";
        document.getElementById("optimizationsCounter").innerHTML = "0 Optimizations";

        document.getElementById("highEfficiencyGathererButton").style.display = "none";
        document.getElementById("highEfficiencyGathererCounter").style.display = "none";
        document.getElementById("highEfficiencyGathererCounter").innerHTML = "0 High-Efficiency Auto-Gatherers";

        // Reset other sections similarly...

        console.log("Game has been reset.");
        location.reload(); // Reload the page to ensure all changes take effect
    }
}

function autoSave() {
    // Save the game every 5 minutes by default
    setInterval(() => {
        saveGame();
